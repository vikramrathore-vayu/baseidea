...
const translations = {
  en: {
    ...
    storyText:
      "Founder Mr. Vikram Rathore saw that India has a big health access problem, especially in rural areas where people cannot travel for every small health issue. DOCAI is his online-first solution to collect symptoms, give safe AI guidance, store records, capture WhatsApp leads and route serious cases to doctors.",
  ...
  hi: {
    ...
    storyText:
      "Founder Mr. Vikram Rathore ने देखा कि India में health access की बहुत बड़ी problem है, खासकर rural areas में जहां लोग हर छोटी health issue के लिए travel नहीं कर सकते। DOCAI इसी problem का online-first solution है: symptoms collect, safe AI guidance, records, WhatsApp leads और serious cases को doctors तक route करना।",
  ...
  hinglish: {
    ...
    storyText:
      "Founder Mr. Vikram Rathore ne dekha ki India me health access badi problem hai, specially rural areas me. DOCAI isi issue ka online-first solution hai: symptoms collect, AI guidance, records, WhatsApp leads aur doctor routing.",
  ...
  mr: {
    ...
    storyText:
      "Founder Mr. Vikram Rathore यांनी पाहिले की India मध्ये health access ही मोठी problem आहे, specially rural areas मध्ये. DOCAI हा त्याच issue चा online-first solution आहे: symptoms collect, AI guidance, records, WhatsApp leads आणि doctor routing.",
...
function recordsToCsv(rows = []) {
  const header = ["Name", "Phone", "Age", "Village/City", "Symptoms", "Duration", "Risk", "Language", "Created At"];
...
function downloadRecordsCsv(rows = [], filename = "docai-records.csv") {
...
function recordsWhatsAppMessage(rows = []) {
  if (!rows.length) return "DOCAI records are empty.";
  return `DOCAI Records Summary\nTotal Records: ${rows.length}\n\n` + rows.slice(-10).reverse().map((r, i) =>
    `${i + 1}. ${r.name || "Patient"} | ${r.phone || "No phone"} | ${r.city || "No city"} | Risk: ${r.risk || "NA"} | Symptoms: ${r.symptoms || "NA"}`
  ).join("\n");
}
...
function Header({ lang, setLang, t }) {
  ...
  return (
    <>
      <header className={`z-30 flex items-center justify-between px-4 pt-4 sm:px-8 md:px-12 md:pt-6 ${onHome ? "absolute inset-x-0 top-0" : "relative pb-5"}`}>
        <motion.div variants={fadeDown} initial="hidden" animate="visible" custom={0}>
          <Logo />
        </motion.div>

        <div className="flex items-center gap-2 md:hidden">
          <Link to="/#story" className="rounded-full border border-black/10 bg-white/70 px-3 py-2 text-[10px] font-semibold uppercase tracking-widest text-black shadow-sm backdrop-blur">
            Story
          </Link>
          <Link to="/pricing" className="rounded-full px-3 py-2 text-[10px] font-semibold uppercase tracking-widest text-white shadow-sm" style={{ background: `linear-gradient(135deg, ${PINK}, ${ACCENT})` }}>
            Plan
          </Link>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <motion.button variants={fadeDown} initial="hidden" animate="visible" custom={5} onClick={() => setMenuOpen(true)} className="flex h-9 w-9 items-center justify-center rounded-full bg-black text-white shadow-lg" aria-label="Open menu">
            <Menu size={18} />
          </motion.button>
        </div>
      </header>
...
function HomePage({ lang, setLang, t }) {
  return (
    <>
      <section className="relative min-h-screen overflow-hidden font-inter text-black" style={{ background: PREMIUM_GRADIENT }}>
        <video className="absolute inset-0 h-full w-full object-cover opacity-70 md:opacity-100" src={VIDEO_URL} autoPlay loop muted playsInline aria-hidden="true" />
        <div className="absolute inset-0 bg-gradient-to-b from-[#fff4d8]/60 via-[#ffd6f2]/35 to-[#ffffff]/70 md:from-white/10 md:via-transparent md:to-white/20" />
        ...
        <div className="relative z-10 flex min-h-screen flex-col pt-20">
          <section className="flex flex-1 items-center justify-end px-5 py-8 sm:px-8 md:px-12 md:py-0">
            <div className="grid grid-cols-3 gap-3 rounded-[1.5rem] border border-white/40 bg-white/35 p-3 text-right shadow-lg backdrop-blur-md sm:flex sm:gap-8 sm:p-0 sm:bg-transparent sm:shadow-none sm:border-0 md:gap-10">
              {[
...
function LandingSections({ t }) {
  ...
  return (
    <div className="font-inter text-black" style={{ background: PREMIUM_GRADIENT }}>
      <section id="story" className="px-5 py-20 sm:px-8 md:px-12">
        <div className="mx-auto max-w-5xl rounded-[2rem] border border-white/50 bg-white/55 p-7 shadow-xl backdrop-blur md:p-12">
          <SectionTitle eyebrow="Founder Story" title={t.storyTitle} text={t.storyText} />
        </div>
      </section>
      ...
      <section id="workflow" className="px-5 py-20 text-white sm:px-8 md:px-12" style={{ background: `linear-gradient(135deg, #09000f, ${ACCENT}, #2b061b)` }}>
      ...
      <section id="feedback" className="px-5 pb-20 sm:px-8 md:px-12">
        <div className="mx-auto max-w-5xl rounded-[2rem] border border-white/50 bg-white/60 p-8 shadow-xl backdrop-blur md:p-12">
...
function InfoCard({ Icon, title, text }) {
  return (
    <div className="rounded-[1.5rem] border border-white/60 bg-white/70 p-6 shadow-xl backdrop-blur">
      <div className="flex h-12 w-12 items-center justify-center rounded-full" style={{ background: `linear-gradient(135deg, #ffe7f8, #fff1c7)`, color: ACCENT }}><Icon size={22} /></div>
...
function PageShell({ children, lang, setLang, t }) {
  return <div className="min-h-screen font-inter text-black" style={{ background: PREMIUM_GRADIENT }}><Header lang={lang} setLang={setLang} t={t} /><main className="px-5 py-8 sm:px-8 md:px-12">{children}</main></div>;
}
  ...
  return (
    <PageShell lang={lang} setLang={setLang} t={t}>
      <div className="mx-auto grid max-w-5xl overflow-hidden rounded-[2rem] bg-white shadow-xl md:grid-cols-2">
        <div className="p-8 text-white md:p-12" style={{ background: `linear-gradient(135deg, #09000f, ${ACCENT}, ${PINK})` }}>
      ...
      <div className="mx-auto max-w-7xl">
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
          ...
          <div className="flex flex-wrap gap-2">
            <button onClick={() => downloadRecordsCsv(records, "docai-patient-records.csv")} className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/70 px-4 py-3 text-xs font-semibold uppercase tracking-widest text-black shadow-sm backdrop-blur">
              <Download size={16} /> CSV / Sheets
            </button>
            <a href={makeWhatsAppLink(recordsWhatsAppMessage(records))} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full px-4 py-3 text-xs font-semibold uppercase tracking-widest text-white shadow-sm" style={{ background: `linear-gradient(135deg, ${PINK}, ${ACCENT})` }}>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-3">
          <div className="rounded-[2rem] border border-white/60 bg-white/70 p-6 shadow-xl backdrop-blur lg:col-span-2">
          ...
          <div className="rounded-[2rem] p-6 text-white shadow-xl" style={{ background: `linear-gradient(135deg, #09000f, ${ACCENT}, ${PINK})` }}>
...
function Metric({ icon: Icon, label, value }) {
  return <div className="rounded-[1.5rem] border border-white/60 bg-white/70 p-6 shadow-xl backdrop-blur"><Icon style={{ color: ACCENT }} /><p className="mt-5 text-xs font-semibold uppercase tracking-widest text-black/50">{label}</p><h3 className="mt-2 text-3xl font-semibold uppercase tracking-wide">{value}</h3></div>;
}
    ...
    <PageShell lang={lang} setLang={setLang} t={t}>
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-6 flex flex-wrap justify-center gap-3">
          <button onClick={() => downloadRecordsCsv(records, "docai-all-records.csv")} className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/70 px-5 py-3 text-xs font-semibold uppercase tracking-widest text-black shadow-sm backdrop-blur">
            <Download size={16} /> Download CSV For Google Sheets
          </button>
          <a href={makeWhatsAppLink(recordsWhatsAppMessage(records))} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full px-5 py-3 text-xs font-semibold uppercase tracking-widest text-white shadow-sm" style={{ background: `linear-gradient(135deg, ${PINK}, ${ACCENT})` }}>
            <MessageCircle size={16} /> Share Records On WhatsApp
          </a>
        </div>
        ...
        <div className="mt-8 overflow-hidden rounded-[2rem] border border-white/60 bg-white/70 shadow-xl backdrop-blur">
